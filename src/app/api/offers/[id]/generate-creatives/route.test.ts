...
const MODEL_INTENT_BUCKET_INTENT = '商品型号意图导向 - 聚焦当前商品型号/系列，关键词统一完全匹配'
...
describe('POST /api/offers/:id/generate-creatives', () => {
  beforeEach(() => {
    ...
    generatorFns.generateAdCreative.mockResolvedValue({
      ...
      callouts: [],
      sitelinks: [],
      theme: '商品型号意图导向 - 聚焦当前商品型号/系列，关键词统一完全匹配',
  ...
  it('normalizes legacy creativeType/bucket inputs to canonical model intent and persists canonical fields', async () => {
    ...
    expect(generatorFns.generateAdCreative).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        bucketIntentEn: '聚焦当前商品型号/系列，关键词统一完全匹配',
  ...
  it('keeps AI candidates when bucket seeds are available instead of bucket-only overwrite', async () => {
    ...
    expect(selectCall.keywords).toEqual(expect.arrayContaining([
      'brandx x200 vacuum',
    ]))

    const aiCandidate = (selectCall.keywordsWithVolume || []).find(
      (item: any) => item.keyword === 'brandx x200 vacuum'
    )
    expect(aiCandidate).toBeTruthy()
    expect(String(aiCandidate.source || '').toUpperCase()).toBe('AI_GENERATED')
  })
})
