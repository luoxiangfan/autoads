...
function getPromptKeywordPoolHint(bucket: PromptBucket | null): string {
  ...
  if (bucket === 'B') {
    return '关键词必须围绕已验证的型号/系列长尾词组织，并按完全匹配思路使用，禁止退化为纯品牌词或泛品类词。'
  }
  if (bucket === 'D') {
...
export function buildIntentStrategySection(
  ...
  const bucketStrategies = {
    ...
    B: {
      name: '商品型号导向 / model_intent',
      keywordRatio: 55,
      intentRatio: 45,
      focus: '围绕型号/系列锚点做长尾精准覆盖，承接强购买意图，优先用可直接购买的精确表达',
      distribution: '55% 型号/系列关键词密集型 + 20% 规格/证据型 + 15% 问答式（适配/差异点） + 10% CTA',
      keywordRule: '关键词必须带型号或系列锚点，统一按完全匹配思路组织，不得退化成泛品类词',
      keywordExample: '"Eufy X10 Pro Omni", "Eufy X10 Pro Robot Vacuum"',
      intentRule: '强调具体型号、系列、规格和购买动作，避免泛场景文案',
      matchRule: '关键词全部按 EXACT 思路组织，只允许品牌+型号/系列+品类的高精度长尾词'
    },
    D: {
