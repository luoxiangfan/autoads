 ...
 * 4. 支持 3 个差异化创意生成（品牌意图 / 商品型号意图 / 商品需求意图）
 *
 * 关键词分层策略：
...
import { getKeywordSourcePriorityScore as getUnifiedKeywordSourcePriorityScore } from './creative-keyword-source-priority'
...
function buildVerifiedSourceKeywordData(offer: Offer): VerifiedSourceKeywordMap {
  ...
  const candidateMap: VerifiedSourceKeywordMap = {
    TITLE_EXTRACT: createKeywordData(sourcePool.titleKeywords, 'TITLE_EXTRACT'),
    ABOUT_EXTRACT: createKeywordData(sourcePool.aboutKeywords, 'ABOUT_EXTRACT'),
    PARAM_EXTRACT: createKeywordData(sourcePool.paramKeywords, 'PARAM_EXTRACT'),
    HOT_PRODUCT_AGGREGATE: createKeywordData(sourcePool.hotProductKeywords, 'HOT_PRODUCT_AGGREGATE'),
    PAGE_EXTRACT: createKeywordData(sourcePool.pageKeywords, 'PAGE_EXTRACT'),
  }
...
export async function generateOfferKeywordPool(
  ...
  const pageType = resolveOfferPageType(offer)
  const verifiedSourceKeywords = buildVerifiedSourceKeywordData(offer)
...
const CANONICAL_PLATFORM_PATTERN = /\b(amazon|walmart|ebay|etsy|aliexpress|temu)\b/i
const CANONICAL_INFO_QUERY_PATTERN = /\b(what is|meaning|tutorial|guide|manual|how to|instructions?)\b/i
const CANONICAL_REVIEW_COMPARE_PATTERN = /\b(review|reviews|comparison|compare|vs)\b/i
const CANONICAL_STORE_NAV_PATTERN = /\b(official store|store locator|near me)\b/i
const CANONICAL_PROMO_PATTERN = /\b(discount|coupon|cheap|sale|deal|offer|promo|price|cost)\b/i
...
function shouldDropCanonicalKeyword(
  ...
  if (CANONICAL_PLATFORM_PATTERN.test(keyword)) return true
  if (CANONICAL_INFO_QUERY_PATTERN.test(keyword)) return true
  if (CANONICAL_REVIEW_COMPARE_PATTERN.test(keyword)) return true
  ...
  if (CANONICAL_STORE_NAV_PATTERN.test(keyword) && !hasDemandAnchor) return true
  if (CANONICAL_PROMO_PATTERN.test(keyword) && !hasDemandAnchor) {
    return true
  }
  if (creativeType === 'model_intent' && !hasModelAnchor) return true
...
function sortCanonicalKeywords(
  ...
  if (creativeType === 'product_intent') {
    const nonPureBrand = ranked.filter((item) => !isPureBrandPoolKeyword(item, pureBrandKeywords))
    if (nonPureBrand.length > 0) {
      const pureBrandFallback = ranked.find((item) => isPureBrandPoolKeyword(item, pureBrandKeywords))
      return pureBrandFallback
...
function buildCanonicalSourceFirstBucketKeywords(
  ...
  const mergedWithFallback = bucket === 'D'
    ? mergeKeywordDataLists([merged, pool.brandKeywords])
    : merged

  if (bucket === 'A') {
    return sortCanonicalKeywords(mergedWithFallback, 'brand_intent', pureBrandKeywords)
  }

  if (bucket === 'B') {
    return sortCanonicalKeywords(mergedWithFallback, 'model_intent', pureBrandKeywords)
  }

  return sortCanonicalKeywords(mergedWithFallback, 'product_intent', pureBrandKeywords)
}
...
function buildLegacyProjectedCanonicalBucketKeywords(
  ...
  if (bucket === 'B') {
    ...
    const modelAnchored = merged.filter((item) => hasModelAnchorEvidence({ keywords: [item.keyword] }))
...
async function applyOfferContextToCanonicalKeywords(params: {
  ...
  if (!creativeType || keywords.length === 0) return keywords
  ...
  return filterCreativeKeywordsByOfferContextDetailed({
    offer: offerContext,
  ...
  switch (bucket) {
    ...
    case 'B':
      return {
        ...
        intent: isStore ? '热门商品型号意图' : '商品型号意图',
        intentEn: isStore ? 'Store Model Intent' : 'Model Intent'
      }
    case 'C':
      return {
        // 🔧 向后兼容：旧版 C 桶在 KISS-3 类型方案中等价于 B（商品型号意图）
...
export async function getKeywordsByLinkTypeAndBucket(
  ...
  if (linkType !== 'product' || !effectiveCreativeType || bucketInfo.keywords.length === 0) {
    return bucketInfo
  }
