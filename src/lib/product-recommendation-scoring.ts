...
// 结构化 JSON 任务在 Gemini 3 下仍可能消耗一定 thinking token，给到更安全的输出预算
const PRODUCT_SCORE_AI_MAX_OUTPUT_TOKENS = 4096
...
function buildCombinedProductScorePrompt(
  ...
  return [
    'Analyze this affiliate product and return JSON only.',
    `Current month: ${currentMonth}`,
    `Product name: ${product.product_name || 'Unknown'}`,
    `Brand: ${product.brand || 'Unknown'}`,
    `Price: ${product.price_amount ? `$${product.price_amount}` : 'Unknown'}`,
    '',
    'Return one JSON object with these two top-level fields:',
    '1. seasonality: seasonality, holidays, isPeakSeason, monthsUntilPeak, reasoning.',
    '2. productAnalysis: category, targetAudience, pricePositioning, useScenario, productFeatures, reasoning.',
    '',
    'Rules:',
    '- Base the answer on the product identity and common market understanding.',
    '- Prefer conservative judgments when the title is ambiguous.',
    '- monthsUntilPeak must be between 0 and 12.',
    '- targetAudience, useScenario, and productFeatures must stay within the provided enums.',
    '- Output must be one valid JSON object only (no markdown code block, no extra commentary).',
    '- Keep both reasoning fields concise (prefer <= 80 characters each).',
  ].join('\n')
}
...
function buildCombinedProductScoreRetryPrompt(
  ...
  return [
    ...
    'Do not use markdown, do not wrap with ```.',
...
function calculateMarketFitScore(product: AffiliateProduct): DimensionScore {
  ...
  const landingPageScore = calculateLandingPageScore(product.product_url)
 ...
 * 落地页类型评分
 */
function calculateLandingPageScore(productUrl: string | null): number {
  if (!productUrl) return 30 // 无URL

  const urlLower = productUrl.toLowerCase()

  // Amazon产品页
  if (urlLower.includes('amazon.') && urlLower.includes('/dp/')) return 100

  // Amazon店铺页
  if (urlLower.includes('amazon.') && (urlLower.includes('/stores/') || urlLower.includes('/store/'))) return 85

  // 独立站产品页 (包含product关键词)
  if (urlLower.includes('product')) return 60
...
async function analyzeProductScoreCombined(
  ...
  return await withCache(
    ...
    async () => {
      const responseSchema: ResponseSchema = {
        ...
        properties: {
          seasonality: {
            type: 'OBJECT',
            properties: {
              seasonality: { type: 'STRING', enum: ['winter', 'summer', 'spring', 'fall', 'all-year'] },
              holidays: { type: 'ARRAY', items: { type: 'STRING' } },
              isPeakSeason: { type: 'BOOLEAN' },
              monthsUntilPeak: { type: 'INTEGER', minimum: 0, maximum: 12 },
              reasoning: { type: 'STRING' }
            },
            required: ['seasonality', 'holidays', 'isPeakSeason', 'monthsUntilPeak', 'reasoning']
          },
          productAnalysis: {
            type: 'OBJECT',
            properties: {
              category: {
                type: 'STRING',
                enum: ['electronics', 'clothing', 'home', 'sports', 'beauty', 'toys', 'books', 'food', 'automotive', 'health', 'other']
              },
              targetAudience: {
                type: 'ARRAY',
                items: { type: 'STRING', enum: ['male', 'female', 'kids', 'elderly', 'unisex'] }
              },
              pricePositioning: {
                type: 'STRING',
              ...
              useScenario: {
                type: 'ARRAY',
                items: { type: 'STRING', enum: ['indoor', 'outdoor', 'sports', 'office', 'travel', 'daily', 'party', 'professional'] }
              },
              productFeatures: {
                type: 'ARRAY',
                items: { type: 'STRING', enum: ['portable', 'durable', 'fashionable', 'practical', 'innovative', 'eco-friendly', 'smart', 'luxury'] }
              },
              reasoning: { type: 'STRING' }
            },
            required: ['category', 'targetAudience', 'pricePositioning', 'useScenario', 'productFeatures', 'reasoning']
          }
        },
      ...
      let analysis: CombinedProductScoreAnalysis
      try {
        analysis = parseCombinedProductScoreAnalysis(firstResult.text)
      } catch (parseError) {
        console.warn(
          `⚠️ 商品评分合并分析解析失败，触发一次重试: ${(parseError as Error).message}`
        )
