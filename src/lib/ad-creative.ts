...
export async function createAdCreative(
  ...
  const shouldTrustProvidedKeywordSelection = executableKeywords.length > 0
  const selectedKeywords = shouldTrustProvidedKeywordSelection
    ? undefined
    : selectCreativeKeywords({
  ...
  const finalKeywords = shouldTrustProvidedKeywordSelection
    ? executableKeywords
    : (selectedKeywords?.keywords || normalizedKeywords)
  const finalKeywordsWithVolume: KeywordWithVolume[] = shouldTrustProvidedKeywordSelection
    ? normalizedKeywordsWithVolume
    : (
      (selectedKeywords?.keywordsWithVolume as KeywordWithVolume[] | undefined)
      || normalizedKeywordsWithVolume
    )
  const normalizedAdStrength = data.adStrength && typeof data.adStrength === 'object'
    ? { ...data.adStrength }
    : undefined
  const adStrengthAudit = normalizeCreativeAuditForCompat(
    (normalizedAdStrength as any)?.audit || (normalizedAdStrength as any)?.keywordSourceAudit
  )
  if (normalizedAdStrength && adStrengthAudit) {
    ;(normalizedAdStrength as any).audit = adStrengthAudit
    ;(normalizedAdStrength as any).keywordSourceAudit = adStrengthAudit
  }
