...
export function resolveAdCreativePromptKeywordPlan(input: {
  ...
  const highIntentTitleAboutSeeds = dedupeKeywordSeeds(
    filterHighIntentKeywordSeeds(
      titleAboutSeedsPolicySafe.items,
...
function extractTitlePriorityHeadlineCandidates(options: {
  ...
  const titleSegments = title
    .split(/\s*[|,:;]\s*/g)
    .map((segment) => normalizeHeadlineCandidateText(segment))
...
export function evaluateStoreModelIntentReadiness(params: {
  ...
  if (verifiedHotProducts.length === 0) {
    return {
      ...
      hotProductModelAnchors: [],
      evidenceSources,
      reason: '店铺热门商品信息不足，无法生成商品型号意图创意：未获取到可验证的热门商品，请先重抓或补充店铺商品数据。',
    }
  }
  ...
  if (hotProductModelAnchors.length === 0) {
    return {
      ...
      hotProductModelAnchors,
      evidenceSources,
      reason: '店铺热门商品信息不足，无法生成商品型号意图创意：未提取到可验证的型号/系列锚点，请先重抓或补充店铺商品数据。',
    }
  }
...
### B（热门商品型号意图）
**目标**: 承接已锁定热门商品型号/系列的强购买意图
- 关键词侧重：品牌 + 热门商品型号/系列 + 品类长尾词
- 表达重点：围绕热门商品型号、系列和具体购买动作
...
- 创意重点: 品牌优先，但必须回到当前商品`,
        'B': `
**📦 产品桶B - 商品型号意图导向 (Model Intent)**
**🎯 核心主题**: 当前商品型号/系列购买意图
**⚠️ 单品聚焦规则 (CRITICAL)**:
- ✅ 广告语和关键词必须围绕这一个产品的型号/系列
- ✅ 关键词必须覆盖品牌 + 型号/系列 + 品类的长尾词
- ✅ 最终关键词统一完全匹配
- ❌ 禁止: 退化成泛品类词、场景词或纯品牌词
- ❌ 禁止: 暗示多产品选择或店铺级文案
- 创意重点: 精准、可投放、强购买意图`,
...
| B | 热门商品型号 | 品牌 + 热门商品型号/系列 + 品类长尾词 | 热门型号/系列 + 购买动作 + 完全匹配 |
...
export function getThemeByBucket(bucket: BucketType, linkType: 'product' | 'store'): string {
  ...
  if (linkType === 'store') {
    const themes: Record<'A' | 'B' | 'D', string> = {
      ...
      'B': '热门商品型号意图导向 - 聚焦店铺热门商品型号/系列，关键词统一完全匹配',
    ...
    const themes: Record<'A' | 'B' | 'D', string> = {
      ...
      'B': '商品型号意图导向 - 聚焦当前商品型号/系列，关键词统一完全匹配',
      'D': '商品需求意图导向 - 聚焦品牌下商品需求、功能、场景和产品线覆盖',
    }
    return themes[normalizedBucket]
  }
...
async function runAdCreativeModelAttempt(params: {
  ...
  const result = validateGeneratedAdCreativeBusinessLimits(parsed)
...
export async function generateAdCreativesBatch(
  ...
  const themes = options?.theme
    ...
    : [
        ...
        'Model Intent - 强调型号/系列购买意图。Headlines优先体现型号、系列或规格锚点，Descriptions突出精准适配与购买动作。',
