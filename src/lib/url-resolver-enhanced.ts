...
async function resolveWithPlaywright(
  ...
  const result = await resolveAffiliateLinkWithPlaywright(affiliateLink, proxyUrl, 5000, targetCountry, userId)
  ...
  while (attempt <= retryConfig.maxRetries) {
    ...
    try {
      ...
      if (resolverMethod === 'playwright') {
        ...
        result = await resolveWithPlaywright(affiliateLink, proxy.url, targetCountry, userId)
        ...
        try {
          ...
          if (blocked) {
            ...
            const playwrightResult = await resolveWithPlaywright(fullResolvedUrl, proxy.url, targetCountry, userId)
            result = {
              ...playwrightResult,
            ...
            if (isTrackingUrl) {
              ...
              const playwrightResult = await resolveWithPlaywright(fullTrackingUrl, proxy.url, targetCountry, userId)
        ...
        try {
          ...
          if (blocked) {
            ...
            if (result.redirectCount === 0 && affiliateLink !== result.finalUrl) {
              ...
              const playwrightResult = await resolveWithPlaywright(affiliateLink, proxy.url, targetCountry, userId)
