...
async function fetchYeahPromosPromotableProductsWithMeta(params: {
  ...
  const sessionCheck = await checkYeahPromosSessionValidForSync(params.userId, 30 * 60 * 1000) // 至少30分钟
  if (!sessionCheck.valid) {
    ...
    throw new Error(
      sessionCheck.isExpired
        ...
        : `YeahPromos 登录态即将过期（剩余 ${remainingMinutes} 分钟），同步任务终止以避免中途失败`
    )
  }
  ...
  if (cachedSummary && hasCachedPlatformStats) {
    ...
    if (statusFilter === 'all') {
      ...
      landingPageStats = lightweightSummary
        ? resolveCachedLandingPageStats(total)
        : (() => {
