  ...
  try {
    ...
    const generationResult = await runCreativeGenerationQualityLoop<Awaited<ReturnType<typeof generateAdCreative>>>({
      ...
      generate: async ({ attempt, retryFailureType }) => {
        ...
        const seedCandidates = Array.isArray(bucketInfo?.keywords)
          ? bucketInfo.keywords as Array<Record<string, any>>
          : []
        ...
        if (
          ...
          && keywordSet.contextFallbackStrategy !== 'filtered'
        ) {
          console.warn(
            keywordSet.contextFallbackStrategy === 'keyword_pool'
        ...
        usedKeywords = mergeUsedKeywordsExcludingBrand({
          usedKeywords,
          candidateKeywords: generatedKeywordsForExclusion,
          brandKeywords,
        })
    ...
    if (placeholderCreativeId) {
      ...
      savedCreative = await createAdCreative(task.userId, offerId, {
        ...
        bucket_intent: (bucketInfo as { intent: string } | null)?.intent || undefined
      })
    }
