...
function isLanguageScriptMismatch(params: {
  ...
  if (isPureBrandKeyword(keyword, pureBrandKeywords)) return false
  ...
  if (scriptFamilies.size === 0) return false

  // Latin-market campaigns: remove obvious non-Latin script drift.
  if (allowedFamilies.size === 1 && allowedFamilies.has('latin')) {
    return hasAnyNonLatinFamily(scriptFamilies)
  }
  ...
  if (
    ...
    && !hasModelLikeToken(normalizeRelevanceTokens(keyword))
  ) {
    return true
  }
...
function computeContextMatchCount(params: {
  ...
  if (usableContext.length === 0) return 0

  const contextSet = new Set(usableContext)
  return keywordTokens.reduce((acc, t) => acc + (contextSet.has(t) ? 1 : 0), 0)
}
...
export function filterKeywordQuality(
  ...
  for (const kw of keywords) {
    ...
    const isPlannerSource = typeof keywordData.source === 'string' && keywordData.source.toUpperCase().startsWith('KEYWORD_PLANNER')
