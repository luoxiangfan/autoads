...
function collectRecordTextList(value: unknown, limit = 10): string[] {
  ...
  for (const [key, raw] of Object.entries(value).slice(0, limit)) {
    ...
    if (typeof raw === 'string' || typeof raw === 'number') {
      pushUniqueText(output, `${normalizedKey} ${raw}`, limit)
      continue
    }
    ...
    if (Array.isArray(raw)) {
      ...
      if (joined) {
        pushUniqueText(output, `${normalizedKey} ${joined}`, limit)
      }
    }
  }
...
function stripSourceTextLabel(text: string): string {
  return text
    .replace(/^(model|series|version|style|color|size|option|feature|features|use case|scenario|summary|theme)\s*[:\-]\s*/i, '')
    .replace(/\s+/g, ' ')
    .trim()
}
