-- MCC 多租户隔离支持 (PostgreSQL)
-- 2026-03-27

-- 1. 添加租户隔离相关字段到 mcc_accounts 表
ALTER TABLE mcc_accounts ADD COLUMN IF NOT EXISTS tenant_id TEXT;
ALTER TABLE mcc_accounts ADD COLUMN IF NOT EXISTS is_tenant_isolated BOOLEAN DEFAULT FALSE;
...
VALUES ('215_mcc_tenant_isolation', CURRENT_TIMESTAMP)
ON CONFLICT (name) DO NOTHING;

-- 说明：
-- - tenant_id: 租户标识符（可以是公司 ID、团队 ID 等）
-- - is_tenant_isolated: 是否启用租户隔离（true=启用，false=不启用）
