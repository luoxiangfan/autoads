...
CREATE TABLE IF NOT EXISTS campaign_backups (
  ...
  -- 备份数据
  campaign_data JSONB NOT NULL,           -- JSON 格式的完整 campaign 数据
  campaign_config JSONB,                  -- campaign_config JSON
  
  -- 备份元数据
  backup_type VARCHAR(50) NOT NULL,       -- auto_created | sync_initial | sync_7day | manual
  backup_reason TEXT,                     -- 备份原因描述
  
  -- 发布状态
  publish_status VARCHAR(50) DEFAULT 'pending',  -- pending | published | failed | skipped
...
DO $$
BEGIN
  -- 检查并添加 campaign_id 外键
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'fk_campaign_backups_campaign_id'
  ) THEN
    ALTER TABLE campaign_backups
    ADD CONSTRAINT fk_campaign_backups_campaign_id
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'fk_campaign_backups_user_id'
  ) THEN
    ALTER TABLE campaign_backups
    ADD CONSTRAINT fk_campaign_backups_user_id
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'fk_campaign_backups_offer_id'
  ) THEN
    ALTER TABLE campaign_backups
    ADD CONSTRAINT fk_campaign_backups_offer_id
...
VALUES ('009_add_campaign_backups_table', NOW())
