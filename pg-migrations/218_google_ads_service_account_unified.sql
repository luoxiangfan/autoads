-- ==========================================
-- Google Ads 服务账号统一配置 (PostgreSQL)
-- ==========================================
-- 管理员统一配置服务账号，用户选择使用
-- Created: 2026-03-31
-- Migration ID: 218
...
ALTER TABLE mcc_accounts ADD COLUMN IF NOT EXISTS auth_type TEXT DEFAULT 'oauth';  -- 'oauth' 或 'service_account'

-- 服务账号相关字段（当 auth_type='service_account' 时使用）
ALTER TABLE mcc_accounts ADD COLUMN IF NOT EXISTS service_account_email TEXT;
ALTER TABLE mcc_accounts ADD COLUMN IF NOT EXISTS service_account_key TEXT;  -- 加密存储
ALTER TABLE mcc_accounts ADD COLUMN IF NOT EXISTS service_account_id TEXT;  -- 服务账号唯一标识
